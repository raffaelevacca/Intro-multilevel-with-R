---
title: "Introduction to multilevel analysis in R with `tidyverse` and `lme4`"
author: 
  name: "Raffaele Vacca"
...
**R packages and references**. I focus on the [`lme4`](https://github.com/lme4/lme4) package for (Restricted) Maximum Likelihood Estimation of linear multilevel models [@bates_fitting_2015; @bates_lme4:_2012] and on its integration with the [`tidyverse`](https://www.tidyverse.org/), a collection of R packages for data science (including `dplyr`, `ggplot2`, and `purrr`) with a common language and set of principles [@wickham_r_2017]. I draw from the presentation of (linear) multilevel modeling by @fox_fitting_2018, and from discussion of statistical modeling with R by Wickham and Grolemund [-@wickham_r_2017, particularly Ch. 20]. The data come from the [`MathAchieve`](https://rdrr.io/cran/nlme/man/MathAchieve.html) and [`MathAchSchool`](https://rdrr.io/cran/nlme/man/MathAchSchool.html) data frames in the [`nlme`](https://cran.r-project.org/web/packages/nlme/index.html) package (see the links for data documentation).

**Further reading and resources**. For statistical theory and more in-depth treatment of the multilevel models presented in this introduction, see @goldstein_multilevel_2010; @rasbash_lemma:_2008; and @snijders_multilevel_2012. For more information about the R implementation of multilevel models, including different packages and estimation methods, see Ben Bolker's [FAQ page](http://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#introduction) about (Generalized) Linear Mixed Models.
...
* _What do you notice about intercepts and slopes of the red regression lines? Are they constant among schools? _
